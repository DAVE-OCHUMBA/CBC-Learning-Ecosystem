...
const cc = (score) => score >= 80 ? T.teal : score >= 60 ? T.gold : T.rust;
...
function AttendanceTab() {
  ...
  const markAll = (s) => { setStatuses(STUDENTS.map(() => s)); setSaved(false); };
...
function AssessmentsTab() {
  ...
  const avg = (scores) => scores ? Math.round(scores.reduce((a,b)=>a+b,0)/scores.length) : null;
  const pct = (scores, max) => scores ? Math.round((avg(scores)/max)*100) : null;
                ...
                {score !== null ? (
                  <>
                    ...
                    <div style={{ fontFamily:"'DM Mono',monospace", fontSize:12, fontWeight:700,
                      color:cc(p), minWidth:44, textAlign:"right" }}>{score}/{a.max}</div>
              ...
              {a.scores && (
                <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                  <div style={{ flex:1, height:5, background:T.bg3, borderRadius:3, overflow:"hidden" }}>
                    <div style={{ width:`${p}%`, height:"100%", background:cc(p), borderRadius:3 }} />
                  </div>
                  <span style={{ fontFamily:"'DM Mono',monospace", fontSize:12, fontWeight:700, color:cc(p), minWidth:36, textAlign:"right" }}>{p}%</span>
...
function LessonPlanTab() {
  ...
  return (
    <div>
      ...
      {[
        { label:"Topic / Title", key:"topic", multiline:false },
        { label:"CBC Strand", key:"strand", multiline:false },
        { label:"Learning Objective", key:"objective", multiline:false },
        { label:"Materials & Resources", key:"materials", multiline:false },
        { label:"Introduction (Hook)", key:"intro", multiline:true },
        { label:"Main Activities", key:"main", multiline:true },
        { label:"Plenary / Closure", key:"plenary", multiline:true },
        { label:"Homework / Follow-up", key:"homework", multiline:false },
      ].map(f => (
